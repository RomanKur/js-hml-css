...
xhr.onload = function(){
    ...
    for (var i = 0; i < jdata.hardText.length; i++) {
        document.getElementById("hardware").innerHTML += "<p><a href="+jdata.hardText[i].hardlink+">"+jdata.hardText[i].hardheading+"</a></p>";
    }
    for (var j = 0; j < jdata.perText.length; j++) {
      document.getElementById("perepherals").innerHTML += "<p><a href="+jdata.perText[j].perlink+">"+jdata.perText[j].perheading+"</a></p>";
    }
};
...
function Calendar4(id, year, month) {
            ...
            if (new Date(D.getFullYear(),D.getMonth(),1).getWeek() < 10) {
              var calendar = '<tr><td>0' + new Date(D.getFullYear(),D.getMonth(),1).getWeek();
            }else{
              var calendar = '<tr><td>' + new Date(D.getFullYear(),D.getMonth(),1).getWeek();
            }
            if (DNfirst != 0) {
